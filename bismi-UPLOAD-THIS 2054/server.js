...
const server=http.createServer(async(req,res)=>{
  ...
  if(url==='/api/catalogue'&&req.method==='POST'){const body=await parseBody(req);if(db){try{await db.collection('catalogue').replaceOne({key:'main'},{key:'main',...body,updatedAt:new Date()},{upsert:true});emit('catalogue-updated',{});return jsonRes(res,{ok:true});}catch(e){console.error('Catalogue save error:',e.message);return jsonRes(res,{ok:false,error:e.message});}}return jsonRes(res,{ok:false,error:'No database connection'});}
  ...
  // Photo upload endpoint - saves to filesystem
  if(url==='/api/upload-photo'&&req.method==='POST'){
    ...
    req.on('end',()=>{
      try{
        ...
        if(!fileData||!filename){return jsonRes(res,{ok:false,error:'No file'});}
        ...
        return jsonRes(res,{ok:true,url:fileUrl});
      }catch(e){return jsonRes(res,{ok:false,error:e.message});}
    });
    return;
  }
