  ...
  container.innerHTML=list.map(r=>{
    ...
    return `<div class="request-card">
      ...
      <div class="request-details">
        <span>🎂 ${getCakeTypeName(r.cakeType)} — ${getFlavorName(r.baseFlavor)}</span>
        <span>⚖️ ${r.minWeight||'—'} kg | 📅 ${formatDate(r.orderDate)} ${r.orderTime||''}</span>
        <span>📞 ${r.phone||'—'} | 💳 ${r.paymentMethod||'—'}</span>
      </div>
...
function syncFromServer(){
  fetch('/api/orders').then(r=>r.json()).then(data=>{
    if(Array.isArray(data)&&data.length>0){
      ...
      data.forEach(o=>{ if(!localIds.has(o.id)) orders.push(o); });
      saveData(); renderOrdersList(); updateDashboard(); populateYearFilters();
    }
  }).catch(()=>{});
}

// ── STUB FUNCTIONS (referenced but defined elsewhere or missing) ──
